...
#ifndef LLET_RTREE_MIN
#define LLET_RTREE_MIN std::min
#endif //LLET_RTREE_MIN
#ifndef LLET_RTREE_MAX
#define LLET_RTREE_MAX std::max
#endif //LLET_RTREE_MAX
...
typename RTREE_QUAL::Rect RTREE_QUAL::CombineRect(const Rect* a_rectA, const Rect* a_rectB)
{
  ...
  for(int index = 0; index < NUMDIMS; ++index)
    {
      newRect.m_min[index] = LLET_RTREE_MIN(a_rectA->m_min[index], a_rectB->m_min[index]);
      newRect.m_max[index] = LLET_RTREE_MAX(a_rectA->m_max[index], a_rectB->m_max[index]);
    }
